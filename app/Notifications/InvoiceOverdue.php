...
class InvoiceOverdue extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        $message = (new MailMessage())
            ->error()
            ->subject(__('Invoice Overdue'))
            ->greeting('Hello ' . $this->invoice->customer->name . ',')
            ->line(__('Invoice # {number} (Reference: {reference}) is past due as of {due_date}.', ['number' => $this->invoice->id, 'reference' => $this->invoice->reference, 'due_date' => $this->invoice->due_date ?? now()->toDateString()]))
