...
class PaymentCreated extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        $message = (new MailMessage())
            ->subject(__('Payment # {number} ({company})', ['number' => $this->payment->id, 'company' => $this->payment->company->name]))
            ->greeting('Hello ' . $this->payment->customer->name . ',')
            ->line(__('New payment # {number} (Reference: {reference}) has been created.', ['number' => $this->payment->id, 'reference' => $this->payment->reference]))
