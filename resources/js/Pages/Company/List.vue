...
const indeterminate = computed(() => form.selection.length > 0 && form.selection.length < props.companies.data.length);
...
const deleteRow = (id, force = false) => {
  router.delete(route('companies.destroy' + (force ? '.permanently' : ''), id));
};

const restoreRow = id => {
  router.put(route('companies.restore', id));
};
...
const deleteSelected = force => {
  form.delete(route('companies.destroy.many'), {
    onSuccess: () => {
      form.reset();
    },
    onFinish: () => {
...
const searchNow = () => {
  search
    ...
    .get(route('companies.index'), { preserveState: true });
};
...
<template>
  <AppLayout :title="t('List Companies')">
    <div class="py-8">
          ...
          <div class="sm:rounded-lg">
            <div class="p-6 sm:rounded-t-lg border-b border-gray-200 dark:border-gray-700 block md:flex md:items-center md:justify-between">
              ...
              <div class="mt-4 sm:mt-0 block sm:flex sm:items-center sm:gap-4">
                <Button v-if="$can('create-companies')" :href="route('companies.create')">{{ t('Add New Company') }}</Button>
            ...
            <div class="bg-gray-50 dark:bg-black/50 border-b dark:border-gray-700 sm:rounded-b-lg">
              <div v-if="companies && companies.data && companies.data.length">
                <div class="flex flex-col">
                        ...
                        <table class="min-w-full table-fixed divide-y divide-gray-300 dark:divide-gray-700">
                            ...
                            <tr>
                              <th scope="col" class="relative w-12 px-6 sm:w-16 sm:px-8">
                                <input
                                  ...
                                  :checked="indeterminate || form.selection.length === companies.data.length"
                                  @change="form.selection = $event.target.checked ? companies.data.map(p => p.id) : []"
                          ...
                          <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900">
                            <tr
                              ...
                              :class="[
                                form.selection.includes(company.id) && 'bg-gray-50 dark:bg-gray-800/50',
                                company.deleted_at && 'bg-red-100 dark:bg-red-900',
                              ]"
                            >
                              <td class="relative w-12 px-6 sm:w-16 sm:px-8">
                                <div v-if="form.selection.includes(company.id)" class="absolute inset-y-0 left-0 w-0.5 bg-blue-600"></div>
                                <input
                                  type="checkbox"
                                  :value="company.id"
                              ...
                              <td class="whitespace-nowrap px-3 py-4 text-sm font-medium">
                                <div v-if="company.contact_person">{{ company.contact_person }}</div>
                                <div v-if="company.phone">{{ company.phone }}</div>
                                <div v-if="company.email">{{ company.company }}</div>
                              </td>
                              ...
                              <td class="px-3 py-4 text-sm">
                                <div
                                  ...
                                  v-html="$extra_attributes(company.extra_attributes)"
                                ></div>
                              </td>
                              <td class="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 w-20">
                                <row-actions
                                  ...
                                  :editStr="route('companies.edit', company.id)"
                                />
                              </td>
                            </tr>
                          </tbody>
                  ...
                  <pagination class="m-4" :meta="companies.meta" :links="companies.links" />
                </div>
              </div>
