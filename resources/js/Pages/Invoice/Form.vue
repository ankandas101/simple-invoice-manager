...
const props = defineProps(['edit', 'companies', 'tax_rates', 'note', 'fields', 'customer_fields', 'item_fields', 'quote']);

if (props.quote) {
  if (props.quote.extra_attributes && props.fields) {
    Object.keys(props.quote.extra_attributes)
      .filter(k => !props.fields?.find(f => f.name == k))
      .map(k => delete props.quote.extra_attributes[k]);
  }

  company.value = { value: props.quote.company.id, label: props.quote.company.name };
  customer.value = { value: props.quote.customer.id, label: props.quote.customer.name };
  form.date = dayjs(props.quote.date).format('YYYY-MM-DD'); //props.quote.date.split('T')[0];
  form.due_date = props.quote.due_date ? dayjs(props.quote.due_date).format('YYYY-MM-DD') : null;
  //   form.reference = props.quote.reference;
  form.shipping = props.quote.shipping;
  form.company_id = props.quote.company_id;
  form.customer_id = props.quote.customer_id;
  form.discount = props.quote.order_discount;
  form.taxes = props.quote.taxes.map(t => t.id);
  //   form.status = props.quote.status;
  form.items = props.quote.items.map(i => {
  ...
  form.note = props.quote.note;
  form.attachments = null;
  form.extra_attributes = props.quote.extra_attributes || {};
}
if (props.edit) {
  ...
  form.due_date = props.edit.data.due_date ? dayjs(props.edit.data.due_date).format('YYYY-MM-DD') : null;
  ...
  form.recurring = props.edit.data.recurring == 1;
  form.repeat = props.edit.data.repeat;
  form.create_before = props.edit.data.create_before;
  form.billing = props.edit.data.billing;
  form.attachments = null;
...
const selectItem = (item, index) => {
  let items = form.items.map((i, ii) => {
    if (ii == index) {
      ...
      //   i.selected_taxes = item.taxes.map(t => ({ ...t, value: t.id, label: t.name }));
...
function submit(e) {
  ...
  let url = props.edit ? route('invoices.update', props.edit.data.id) : route('invoices.store');
  form.transform(data => {
    data._method = method;
...
<template>
  <AppLayout :title="props.edit ? t('Edit Invoice') : t('Add New Invoice')">
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
      <FormSection @submitted="submit" autocomplete="off">
        <template #title>{{ props.edit ? t('Edit Invoice') : t('Add New Invoice') }}</template>
        <template #description>
          {{
            t('Please fill the form below to {x}.', {
              x: props.edit ? t('update {x}', { x: t('invoice') }) : t('add {x}', { x: t('invoice') }),
            })
          }}
          <div v-if="quote && quote?.id" class="font-bold mt-2">{{ t('Creating form quotation # {x}.', { x: quote.id }) }}</div>
          <div>
            <Button :href="route('invoices.index')" class="mt-4">{{ t('List Invoices') }}</Button>
          </div>
        </template>
        ...
        <template #form>
              ...
              <div class="w-full flex flex-col divide-y dark:divide-gray-700 border-y dark:border-gray-700">
                      ...
                      <div class="text-sm font-medium flex gap-2 items-center justify-between">
                        <div class="flex-1 relative">
                          ...
                          <template v-if="i.show_details">
                            <TextareaInput :id="'item-details-' + r" v-model="i.details" class="mt-2 text-gray-700 dark:text-gray-300" />
                          </template>
                ...
                >
                  ...
                  <div>
                    ...
                    <div v-if="form.shipping" class="text-right h-6 font-bold">{{ $number(form.shipping) }}</div>
          ...
          <div class="col-span-6 sm:col-span-3">
            <TextInput type="date" v-model="form.due_date" id="due_date" :error="form.errors.due_date" :label="t('Due Date')" />
          </div>
          <div class="col-span-6 sm:col-span-3">
          ...
          <div class="col-span-full">
            <CheckBox
              id="recurring"
              :error="form.errors.recurring"
              v-model:checked="form.recurring"
              :label="t('This is recurring invoice')"
            />
          </div>
          <template v-if="form.recurring">
            <div class="col-span-6 sm:col-span-3">
              <AutoComplete
                ...
                :searchable="false"
                v-model="form.repeat"
                :label="t('Repeat Cycle')"
                :error="form.errors.repeat"
                :suggestions="[
                  { value: 'daily', label: 'Every day (daily)' },
                  { value: 'weekly', label: 'Every 7 days (weekly)' },
                  { value: 'monthly', label: 'Every month (monthly)' },
                  { value: 'quarterly', label: 'Every 3 months (quarterly)' },
                  { value: 'semiannually', label: 'Every 6 months (semiannually)' },
                  { value: 'annually', label: 'Every year (annually)' },
                  { value: 'biennially', label: 'Every 2 years (biennially)' },
                ]"
              />
            </div>
            <div class="col-span-6 sm:col-span-3">
              <TextInput
                ...
                :error="form.errors.create_before"
